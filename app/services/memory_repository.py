...
class MemoryRepository:
    ...
    def list_entries(self, client_id: str, user_id: str | None = None) -> list[MemoryEntry]:
        with self.store.connect() as conn:
            if user_id:
                rows = conn.execute('SELECT * FROM memory_entries WHERE client_id = ? AND (user_id = ? OR user_id IS NULL) ORDER BY priority DESC, updated_at DESC', (client_id, user_id)).fetchall()
            else:
    ...
    def upsert_entry(self, entry: MemoryEntryUpsert) -> MemoryEntry:
        ...
        with self.store.connect() as conn:
            conn.execute(
                '''
                INSERT INTO memory_entries (memory_id, client_id, user_id, type, key, value, priority, confidence, is_active, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    def _row_to_entry(row) -> MemoryEntry:
        return MemoryEntry(memory_id=row['memory_id'], client_id=row['client_id'], user_id=row['user_id'], type=row['type'], key=row['key'], value=row['value'], priority=row['priority'], confidence=row['confidence'], is_active=bool(row['is_active']), updated_at=row['updated_at'])
