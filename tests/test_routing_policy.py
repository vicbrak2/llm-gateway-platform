...
class DummyProvider:
    def __init__(self, name: str, timeout_seconds: float, priority: int) -> None:
        self.config = type(
            "Cfg",
            (),
            {"name": name, "timeout_seconds": timeout_seconds, "priority": priority},
        )()
...
def test_fast_prefers_low_timeout_and_limits_parallelism() -> None:
    ...
    providers = [
        DummyProvider("slow", 5.0, 2),
        DummyProvider("fastest", 1.0, 3),
        DummyProvider("fast", 2.0, 1),
    ]
...
def test_quality_prefers_higher_timeout_profiles() -> None:
    policy = RoutingPolicy(max_parallel_providers=3, complexity_threshold=80)
    providers = [
        DummyProvider("p1", 1.0, 1),
        DummyProvider("p2", 6.0, 3),
        DummyProvider("p3", 3.0, 2),
    ]
...
def test_balanced_switches_for_complex_requests() -> None:
    ...
    providers = [
        DummyProvider("p1", 1.0, 1),
        DummyProvider("p2", 6.0, 2),
        DummyProvider("p3", 3.0, 3),
    ]
